...
def get_file_dict(data_directory:str ) -> dict:
    ...
    net_clu_pairs = [(file, clu) for file in list_of_nets for clu in list_of_clus \
                    if file.split('/')[-1].split(".")[0] in clu.split('/')[-1]]
    ## import collections defaultdict
    from collections import defaultdict
    net_clu_dict = defaultdict(list)
    _ = [net_clu_dict[x[0].split("/")[2].upper()].append(net_clu_pairs[idx]) for idx,x in enumerate(net_clu_pairs)]
    ## dictionaries
    return net_clu_dict
...
def read_clu(file_path:str) -> list:
    ...
    with open(file_path, 'r') as f:
            return [(idx,int(line.strip())) for idx,line in enumerate(f.readlines()) \
                                                                    if line[0]!="*"]
